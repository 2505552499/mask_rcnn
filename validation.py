...
def main(parser_data):
    ...
    det_metric = EvalCOCOMetric(val_dataset.coco, "bbox", "result/det_results.json")
    seg_metric = EvalCOCOMetric(val_dataset.coco, "segm", "result/seg_results.json")
    model.eval()
    with torch.no_grad():
    ...
    save_info(det_metric.coco_evaluator, category_index, "result/det_record_mAP.txt")
    save_info(seg_metric.coco_evaluator, category_index, "result/seg_record_mAP.txt")
...
if __name__ == "__main__":
    ...
    # 数据集的根目录
    parser.add_argument('--data-path', default='datasets/images/', help='dataset root')

    # 训练好的权重文件
    parser.add_argument('--weights-path', default='./save_weights/model_109.pth', type=str, help='training weights')

    # batch size(set to 1, don't change)
    parser.add_argument('--batch-size', default=8, type=int, metavar='N',
                        help='batch size when validation.')
    # 类别索引和类别名称对应关系
    parser.add_argument('--label-json-path', type=str, default="split5_indices.json",)
